...
/**
 * Comprehensive tests for PrestigeCommand.
 *
 * <p>Tests the /ccn prestige command behavior including:
 * - Opening prestige menu
 * - Error handling for players without towns
 * - Permission checks
 * - Async data loading</p>
 */
class PrestigeCommandTest {
  ...
  void setUp() {
    ...
    prestigeCommand = new PrestigeCommand();
  }
  ...
  void getName_returnsCorrectName() {
    assertEquals("prestige", prestigeCommand.getName());
  }

  @Test
  void getSyntax_returnsCorrectSyntax() {
    assertEquals("/ccn prestige", prestigeCommand.getSyntax());
  }

  @Test
  void getArguments_returnsCorrectCount() {
    assertEquals(0, prestigeCommand.getArguments());
  }

  @Test
  void getDescription_returnsNotNull() {
    assertNotNull(prestigeCommand.getDescription());
  }

  @Test
  void getTabCompleteSuggestions_returnsEmptyList() {
    var suggestions = prestigeCommand.getTabCompleteSuggestions(player, "", new String[]{});
  ...
  void perform_playerWithoutTown_sendsErrorMessage() {
    ...
    prestigeCommand.perform(player, new String[]{});
  ...
  void perform_withExtraArguments_ignoresExtraArguments() {
    assertDoesNotThrow(() -> prestigeCommand.perform(player, new String[]{"extra"}));
  }

  @Test
  void perform_withMultipleExtraArguments_ignoresExtraArguments() {
    assertDoesNotThrow(() -> prestigeCommand.perform(player, new String[]{"arg1", "arg2"}));
  }

  @Test
  void perform_doesNotThrowNullPointerException() {
    assertDoesNotThrow(() -> prestigeCommand.perform(player, new String[]{}));
  }
