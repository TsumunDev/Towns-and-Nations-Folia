    ...
    PlayerDataStorage.getInstance()
                    ...
                    try {
                      if (args.length == 1) {
                        ...
                        Optional<Integer> yOpt =
                            CommandExceptionHandler.parseInt(player, args[3], "y coordinate");
                        if (xOpt.isEmpty() || yOpt.isEmpty()) {
                          return;
                        }
                        chunk =
                            player.getLocation().getWorld().getChunkAt(xOpt.get(), yOpt.get());
                      }
        ...
        .exceptionally(
            throwable -> {
              TownsAndNations.getPlugin()
                  .getLogger()
                  .severe("UnclaimCommand failed: " + throwable.getMessage());
              return null;
            });
  }
